...
import { Check, X, UserX, Users, Crown, Trash2 } from 'lucide-react'
...
export default function RoomSettings({
  ...
  const [pendingRequests, setPendingRequests] = useState<UserInfo[]>(initialPendingRequests)
  const [roomUsersList, setRoomUsersList] = useState<UserInfo[]>(initialroomUsersList)
  ...
  const handleRequestAction = async (requestUserId: string, action: 'approve' | 'reject') => {
    try {
      ...
      if (response.ok) {
        setPendingRequests(prev => prev.filter(user => user.userId !== requestUserId))
        if (action === 'approve') {
          const approvedUser = pendingRequests.find(user => user.userId === requestUserId)
          if (approvedUser) {
  ...
  const handleKickUser = async (kickUserId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        setRoomUsersList(prev => prev.filter(user => user.userId !== kickUserId))
        toast({
          title: "Success",
    ...
    <div className="sm:h-[calc(100vh-17px)] border border-white/15 rounded-xl h-[calc(100vh-10px)] overflow-y-auto bg-black">
      <div className="max-w-2xl mx-auto p-4 sm:p-6 space-y-6">
        <div className="flex items-center justify-between sticky top-0 bg-black py-2 z-10">
          <h1 className="text-2xl sm:text-3xl font-bold text-white">Room Settings</h1>
          <Button
            onClick={() => router.back()}
            variant="outline"
            size="sm"
        ...
        <div className="space-y-6">
          {isCreator && (
            <div className="p-5 px-4">
              <Label htmlFor="roomName" className="text-lg mb-2 block text-white">Room Name</Label>
                ...
                <ScrollArea className="h-48 border border-gray-700 rounded-md p-4 bg-gray-800">
                  {pendingRequests.length === 0 ? (
                    ...
                    pendingRequests.map((request) => (
                      <div key={request.userId} className="flex items-center justify-between py-2 border-b border-gray-700 last:border-b-0">
                        <span className="text-white">{request.name}</span>
                        <div className="flex gap-2">
                          <Button
                            size="sm"
                            onClick={() => request.userId && handleRequestAction(request.userId, 'approve')}
                            className="bg-white text-black hover:bg-gray-200 transition-all duration-300"
                          >
                          ...
                          <Button
                            size="sm"
                            onClick={() => request.userId && handleRequestAction(request.userId, 'reject')}
          ...
          <div className="p-5 px-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-white">Room Members</h2>
              <Button
            ...
            {showMemberList && (
              <ScrollArea className="h-48 border border-gray-700 rounded-md p-4 bg-gray-800">
                <div className="border-b px-3 mb-2 border-white/50">Total Members: {roomUsersList.length}</div>
                {roomUsersList.length === 0 ? (
                  <p className="text-gray-400">No members found</p>
                ) : (
                  roomUsersList.map((user) => (
                    <div key={user.userId} className="flex items-center justify-between py-2 px-4 hover:bg-gray-700 rounded-md transition-colors duration-200">
                      ...
                      {isCreator && user.userId !== currentUserId && (
                        <Button
                          size="sm"
                          onClick={() => user.userId && handleKickUser(user.userId)}
